...
class ScalarApp(Application):
    ...
    def _load_user_settings(self):
        ...
        if user_settings is not None:
            ...
            if valid_settings:
                self._set_key(scale_key)
                self._set_offset(scale_offset)
                self._set_mode(scale_mode)
                self._set_scale(scale_index)
                self._set_sound(synth_sound_index)
    ...
    def _set_sound(self, i: int) -> None:
        ...
        if self._synth_sound_name == "flute":
            ...
            self.noise_fm_lp.signals.gain = 120
            self.noise_vol_amp.signals.output_range[0] = 18
            self.noise_vol_amp.signals.output_range[1] = 48
